...
public class ChangePhoneOldActivity extends BaseTitleActivity {
    @BindView(R.id.et01)
    EditText et01;
    @BindView(R.id.et02)
    EditText et02;
    ...
    private void verificationCode(String Phone, String Code) {
        showLoading();
        HttpRequest httpRequest = new HttpRequest(AppUrl.VerificationCode);//验证验证码
        httpRequest.add("Phone", Phone);
        ...
        CallServer.getInstance().request(httpRequest, new HttpResponseListener() {
            ...
            public void onSucceed(String response, Gson gson) {
                closeLoading();
                LogUtil.i("验证验证码" + response);
                BaseHttpResponse info = gson.fromJson(response, BaseHttpResponse.class);
                showToast(info.getMessage());
                if (info.getHttpCode() == 200) {
                    Intent mIntent = new Intent(getContext(), ChangePhoneNewActivity.class);
